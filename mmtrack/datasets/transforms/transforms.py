...
@TRANSFORMS.register_module()
class SeqCropClickTrack(BaseTransform):
    """
    crop with shortest distance for click track

    """
    def __init__(self, 
                 gen_point_factor: Union[int, float],
                 crop_size_factor: List[Union[int, float]], 
                 output_size: List[Union[int, float]]
                 ):
        self.gen_point_factor = gen_point_factor
        self.crop_size_factor = crop_size_factor
        self.output_size = output_size
    
    def generate_random_point_from_bbox(self, bbox: np.ndarray):
        """
        bbox ndarry (x1, y1, x2, y2) (4)
        ...
        a = w / self.gen_point_factor
        b = h / self.gen_point_factor
        ...
        random_x = round((cx + _random_x), 2)
        random_y = round((cy + _random_y), 2)

        return np.array([random_x, random_y])
    
    def generate_jittered_bbox_for_template(self,
                                            img: np.ndarray,
                                            point: np.ndarray):
        """
        ...
        return:
            jittered_box ndarray: (4,)
        """
        point_x, point_y = point[0], point[1]
        img_h, img_w = img.shape[0], img.shape[1]
        l, t, r, b = point_x, point_y, img_w - point_x, img_h - point_y
        half_crop_w = min(l, r)
        half_crop_h = min(t, b)

        half_crop_size = math.ceil(math.sqrt((half_crop_w * 2) * (half_crop_h * 2))) / 2

        crop_x1, crop_y1 = point_x - half_crop_size, point_y - half_crop_size
        crop_x2, crop_y2 = point_x + half_crop_size, point_y + half_crop_size

        return np.array([crop_x1, crop_y1, crop_x2, crop_y2])

    def click_track_crop(self,
                        img: np.ndarray,
                        bbox: np.ndarray,
                        crop_size_factor:int,
                        output_size: int):
        x1, y1, x2, y2 = np.split(bbox, 4, axis=-1)
        bbox_w, bbox_h = x2 - x1, y2 - y1
        cx, cy = x1 + bbox_w / 2., y1 + bbox_h / 2.
        ...
        # 1.4 generate padding mask
        img_h, img_w, _ = img_crop_padded.shape
        padding_mask = np.ones((img_h, img_w))
        end_x, end_y = -x2_pad, -y2_pad
        if y2_pad == 0:
            end_y = None
        if x2_pad == 0:
        ...
        padding_mask = cv2.resize(padding_mask, (output_size, output_size)).astype(np.bool_)

        return img_crop_padded, resize_factor, padding_mask

    def generate_box_point(self,
                     bbox_gt: np.ndarray,
                     bbox_cropped: np.ndarray,
                     resize_factor: float,
                     output_size: float,
        ...
        bbox_out_wh = (bbox_gt[2:4] - bbox_gt[0:2]) * resize_factor
        bbox_out_x1y1 = bbox_out_center - 0.5 * bbox_out_wh
        bbox_out_x2y2 = bbox_out_center + 0.5 * bbox_out_wh
        bbox_out = rearrange([bbox_out_x1y1, bbox_out_x2y2], 'b c -> (b c)')

        # resize random point
        if random_point is not None:
            random_out_point = (output_size - 1) / 2. + (
                random_point - bbox_cropped_center) * resize_factor
        else:
            random_out_point = None

        return bbox_out / output_size if normalize else bbox_out, random_out_point / output_size if normalize else random_out_point 
    ...
    def transform(self, results:dict) -> dict:
        ...
        points = []
        results['padding_mask'] = []
        for i, (img, gt_bbox, jittered_bbox) in enumerate(zip(imgs, gt_bboxes, jittered_bboxes)):
            # crop template with shortest distance between the point and bounding edge
            gt_bbox, jittered_bbox = gt_bbox[0], jittered_bbox[0]
            if i == 0:
                point = self.generate_random_point_from_bbox(gt_bbox)
                jittered_bbox = self.generate_jittered_bbox_for_template(img, point)
            else:
                point = None
            crop_img, resize_factor, padding_mask = self.click_track_crop(
                img, jittered_bbox, self.crop_size_factor[i], self.output_size[i]
            )
            generated_bbox, random_point = self.generate_box_point(
                gt_bbox,
                jittered_bbox,
                resize_factor,
                self.output_size[i],
            ...
            if random_point is not None:
                points.append(rearrange(random_point, 'b -> 1 b'))
            new_imgs.append(crop_img)
            if 'img_shape' in results:
                results['img_shape'][i] = crop_img.shape
            results['gt_bboxes'][i] = rearrange(generated_bbox, 'b -> 1 b')
            results['padding_mask'].append(padding_mask)
    ...
    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f'generate_point_factor={self.gen_point_factor}, '
        repr_str += f'crop_size_factor={self.crop_size_factor}, '
        repr_str += f'output_size={self.output_size})'
        return repr_str